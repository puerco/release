...
func createDraftPR(tag string) (err error) {
	...
	logrus.Debugf(
		...
		git.DefaultGithubOrg, git.DefaultGithubReleaseRepo, git.Master,
...
func createWebsitePR(tag string) error {
	...
	logrus.Debugf(
		"PR params: org: %s, repo: %s, headBranch: %s baseBranch: %s",
		defaultKubernetesSigsOrg, defaultKubernetesSigsRepo, git.Master,
		fmt.Sprintf("%s:%s", releaseNotesOpts.githubOrg, branchname),
	)
...
func fixReleaseNotes(workDir string, releaseNotes *notes.ReleaseNotes) error {
	...
	for pr, note := range releaseNotes.ByPR() {
		...
		fmt.Println(pointIfChanged("ReleaseVersion", note.ReleaseVersion, originalNote.ReleaseVersion), note.ReleaseVersion)
...
func editReleaseNote(pr int, workDir string, originalNote, modifiedNote *notes.ReleaseNote) (shouldRetryEditing bool, err error) {
	...
	if fmt.Sprint(originalNote.ReleaseVersion) == fmt.Sprint(modifiedNote.ReleaseVersion) {
		unalteredFields.ReleaseNote.ReleaseVersion = &originalNote.ReleaseVersion
	} else {
		modifiedFields.ReleaseNote.ReleaseVersion = &modifiedNote.ReleaseVersion
		numChanges++
	}
